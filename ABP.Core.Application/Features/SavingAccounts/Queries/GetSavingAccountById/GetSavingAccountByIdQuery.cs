...
{
    public class GetSavingAccountByIdQuery : IRequest<SavingAccountDto>
    {
        public int Id { get; set; }
    }

    public class GetSavingAccountByIdQueryHandler : IRequestHandler<GetSavingAccountByIdQuery, SavingAccountDto>
    {
        ...
        public async Task<SavingAccountDto> Handle(GetSavingAccountByIdQuery query, CancellationToken cancellationToken)
        {
            var entity = await _repository.GetByIdAsync(query.Id);
            if (entity == null) throw new Exception("SavingAccount not found with this id");

            return _mapper.Map<SavingAccountDto>(entity);
        }
    }
}
