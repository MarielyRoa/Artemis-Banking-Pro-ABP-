...
namespace ABP.Unit.Tests.Features.Loans.Queries.GetAllLoansWithInclude
{
    public class GetAllLoansWithIncludeQueryHandlerTests
    {
        private readonly DbContextOptions<ArtemisBankingAppContext> _dbOptions;

        public GetAllLoansWithIncludeQueryHandlerTests()
        {
            _dbOptions = new DbContextOptionsBuilder<ArtemisBankingAppContext>()
                .UseInMemoryDatabase($"TestDb_GetAllLoansWithIncludeQueryHandler_{Guid.NewGuid()}")
                .Options;
        }
        ...
        public async Task Handle_Should_Return_Expected_Result_When_Successful()
        {
            ...
            var handler = new GetAllLoansWithIncludeQueryHandler(mockloanService.Object, mockuserManager.Object);
            var request = new GetAllLoansWithIncludeQuery();
