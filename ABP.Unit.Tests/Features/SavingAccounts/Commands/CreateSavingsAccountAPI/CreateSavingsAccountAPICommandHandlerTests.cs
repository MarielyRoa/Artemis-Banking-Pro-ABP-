...
namespace ABP.Unit.Tests.Features.SavingAccounts.Commands.CreateSavingsAccountAPI
{
    public class CreateSavingsAccountAPICommandHandlerTests
    {
        private readonly DbContextOptions<ArtemisBankingAppContext> _dbOptions;

        public CreateSavingsAccountAPICommandHandlerTests()
        {
            _dbOptions = new DbContextOptionsBuilder<ArtemisBankingAppContext>()
                .UseInMemoryDatabase($"TestDb_CreateSavingsAccountAPICommandHandler_{Guid.NewGuid()}")
                .Options;
        }
        ...
        public async Task Handle_Should_Return_Expected_Result_When_Successful()
        {
            ...
            var handler = new CreateSavingsAccountAPICommandHandler(mocksavingAccountService.Object, mocktransactionService.Object);
            var request = new CreateSavingsAccountAPICommand();
