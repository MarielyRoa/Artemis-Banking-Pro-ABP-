...
namespace ABP.Unit.Tests.Features.CreditCards.Commands.CreateCreditCardAPI
{
    public class CreateCreditCardAPICommandHandlerTests
    {
        private readonly DbContextOptions<ArtemisBankingAppContext> _dbOptions;

        public CreateCreditCardAPICommandHandlerTests()
        {
            _dbOptions = new DbContextOptionsBuilder<ArtemisBankingAppContext>()
                .UseInMemoryDatabase($"TestDb_CreateCreditCardAPICommandHandler_{Guid.NewGuid()}")
                .Options;
        }
        ...
        public async Task Handle_Should_Return_Expected_Result_When_Successful()
        {
            ...
            var mockcreditCardService = new Mock<ICreditCardService>();

            var handler = new CreateCreditCardAPICommandHandler(mockcreditCardService.Object);
            var request = new CreateCreditCardAPICommand();
