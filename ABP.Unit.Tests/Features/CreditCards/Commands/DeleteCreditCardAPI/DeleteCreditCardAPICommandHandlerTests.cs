...
namespace ABP.Unit.Tests.Features.CreditCards.Commands.DeleteCreditCardAPI
{
    public class DeleteCreditCardAPICommandHandlerTests
    {
        private readonly DbContextOptions<ArtemisBankingAppContext> _dbOptions;

        public DeleteCreditCardAPICommandHandlerTests()
        {
            _dbOptions = new DbContextOptionsBuilder<ArtemisBankingAppContext>()
                .UseInMemoryDatabase($"TestDb_DeleteCreditCardAPICommandHandler_{Guid.NewGuid()}")
                .Options;
        }
        ...
        public async Task Handle_Should_Return_Expected_Result_When_Successful()
        {
            ...
            var handler = new DeleteCreditCardAPICommandHandler(mockcreditCardService.Object);
            var request = new DeleteCreditCardAPICommand();
