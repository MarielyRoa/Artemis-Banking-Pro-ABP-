...
{
    public class UpdateCreditCardLimitCommandHandlerTests
    {
        private readonly DbContextOptions<ArtemisBankingAppContext> _dbOptions;

        public UpdateCreditCardLimitCommandHandlerTests()
        {
            _dbOptions = new DbContextOptionsBuilder<ArtemisBankingAppContext>()
                .UseInMemoryDatabase($"TestDb_UpdateCreditCardLimitCommandHandler_{Guid.NewGuid()}")
                .Options;
        }
        ...
        public async Task Handle_Should_Return_Expected_Result_When_Successful()
        {
            ...
            var handler = new UpdateCreditCardLimitCommandHandler(mockcreditCardService.Object);
            var request = new UpdateCreditCardLimitCommand();
