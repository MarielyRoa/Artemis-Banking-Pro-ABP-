...
namespace ABP.Unit.Tests.Features.Commerces.Queries.GetAllCommerces
{
    public class GetAllCommercesQueryHandlerTests
    {
        ...
        public GetAllCommercesQueryHandlerTests()
        {
            _dbOptions = new DbContextOptionsBuilder<ArtemisBankingAppContext>()
                .UseInMemoryDatabase($"TestDb_GetAllCommercesQueryHandler_{Guid.NewGuid()}")
                .Options;
        }
        ...
        public async Task Handle_Should_Return_Expected_Result_When_Successful()
        {
            ...
            var handler = new GetAllCommercesQueryHandler(mockcommerceRepository.Object);
            var request = new GetAllCommercesQuery();
