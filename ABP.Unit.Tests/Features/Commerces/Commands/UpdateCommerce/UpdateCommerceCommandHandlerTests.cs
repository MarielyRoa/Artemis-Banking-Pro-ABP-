...
namespace ABP.Unit.Tests.Features.Commerces.Commands.UpdateCommerce
{
    public class UpdateCommerceCommandHandlerTests
    {
        ...
        public UpdateCommerceCommandHandlerTests()
        {
            _dbOptions = new DbContextOptionsBuilder<ArtemisBankingAppContext>()
                .UseInMemoryDatabase($"TestDb_UpdateCommerceCommandHandler_{Guid.NewGuid()}")
                .Options;
        }
        ...
        public async Task Handle_Should_Return_Expected_Result_When_Successful()
        {
            ...
            var handler = new UpdateCommerceCommandHandler(mockcommerceRepository.Object);
            var request = new UpdateCommerceCommand();
