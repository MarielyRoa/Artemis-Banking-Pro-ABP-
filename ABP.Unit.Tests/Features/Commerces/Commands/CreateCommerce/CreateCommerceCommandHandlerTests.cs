...
namespace ABP.Unit.Tests.Features.Commerces.Commands.CreateCommerce
{
    public class CreateCommerceCommandHandlerTests
    {
        ...
        public CreateCommerceCommandHandlerTests()
        {
            _dbOptions = new DbContextOptionsBuilder<ArtemisBankingAppContext>()
                .UseInMemoryDatabase($"TestDb_CreateCommerceCommandHandler_{Guid.NewGuid()}")
                .Options;
        }
        ...
        public async Task Handle_Should_Return_Expected_Result_When_Successful()
        {
            ...
            var handler = new CreateCommerceCommandHandler(mockcommerceRepository.Object);
            var request = new CreateCommerceCommand();
